...
bool FrontEnd::buildImage(ov_core::CameraData& camera_data, 
    ...
    FrameType frame_type)
    {
        ...
        cv::Mat combined_frame = frame.clone();
        cv::addWeighted(frame, 1-smoothed_blend_factor, event_frame, smoothed_blend_factor, 0, combined_frame);
        comms_interface_->queueFrameAugmented(combined_frame);
        ...
        else if (frame_type == COMBINED_FRAME)
        {
            ...
            {
                processed_frame = combined_frame.clone();
            }
            else
            {
                processed_frame = frame.clone();
