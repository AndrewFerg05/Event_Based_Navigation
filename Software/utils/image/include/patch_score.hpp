...
//! @todo: Rewrite, use Intel AVX intrinsics
//! Zero Mean Sum of Squared Differences Cost
template<int HalfPatchSize>
class ZMSSD
{
public:
  static constexpr int c_patch_size_ = 2u * HalfPatchSize;
  static constexpr int c_patch_area_ = c_patch_size_ * c_patch_size_;
  static constexpr int c_threshold_  = 2000u * c_patch_area_;
  uint8_t* ref_patch_;
  int sumA_, sumAA_;
  ...
  {
    //! @todo: Why is NEON code deactivated?
#ifdef __ARM_NEON__
    if (false)
    {
      uint16x8_t tpl_x8;
  ...
  static constexpr int threshold() { return c_threshold_; }

  int computeScore(const uint8_t* cur_patch) const
  {
    ...
    return sumAA_ - 2*sumAB + sumBB - (sumA_*sumA_ - 2*sumA_*sumB + sumB*sumB)/c_patch_area_;
  }

  int computeScore(const uint8_t* cur_patch, int stride) const
  {
    ...
    if(c_patch_size_ == 8)
    {
      ...
      const uint8_t* imagepointer = cur_patch;
      const uint8_t* templatepointer = ref_patch_;
    ...
    if(c_patch_size_ == 8)
    {
      ...
      const uint8_t* cur_ptr = cur_patch;
      const uint8_t* ref_ptr = ref_patch_;
...
#endif
    {
      ...
      uint32_t sumBB_uint = 0;
      uint32_t sumAB_uint = 0;
      for(int y=0, r=0; y < c_patch_size_; ++y)
      {
        const uint8_t* cur_patch_ptr = cur_patch + y*stride;
        for(int x=0; x < c_patch_size_; ++x, ++r)
        {
