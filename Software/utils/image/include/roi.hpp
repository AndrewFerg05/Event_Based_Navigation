...
struct RoiBase
{
  ...
  Size<T, DIM> sz; //!< size of the given roi
   ...
   */
  RoiBase(const std::array<T,DIM>& lu, const Size<T,DIM>& sz)
    : pt(lu)
    , sz(sz)
  {
  }
   ...
   */
  Size<T, DIM>& size() {return sz;}
  /**
   * @brief size of the ROI (const)
   */
  const Size<T, DIM>& size() const {return reinterpret_cast<const Size<T, DIM>&>(sz);}
